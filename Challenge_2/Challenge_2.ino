...
//Set a list of global variabels that we use later
//type name
int16_t lastError = 0;
float Kmax = 0.4;
float kp = 0.6 * Kmax; 
...
//"int16_t" and "int" both is for whole numbers, but "int" can have a size of 16, 32 and 64 bits where "int16_t" is set to only fill 16 bits
//"floats" is floating-point numbers

//"unsigned int" is just ints that is non negative, so positive integers or zero

#define NUM_SENSORS 5
unsigned int lineSensorValues[NUM_SENSORS]; //setup and array with a size of 5 to later hold our sensor values
